...
class TestResumeCommand:
    """Click CliRunner tests for the resume subcommand."""
    ...
    def test_resume_loads_state_and_runs(self):
    ...
    def test_resume_run_id_not_found_exits(self):
        ...
        try:
            ...
            assert result.exit_code == 1
            assert "no state found for run_id: deadbeef" in result.stderr
        finally:
            os.unlink(agents_path)

    def test_resume_already_completed_exits(self):
    ...
    def test_resume_no_progress_exits(self):
        ...
        try:
            ...
            assert result.exit_code == 1
            assert "no progress to resume from" in result.stderr
        finally:
            os.unlink(agents_path)

    def test_resume_no_workflow_name_exits(self):
        ...
        try:
            ...
            assert result.exit_code == 1
            assert "no workflow_name" in result.stderr
        finally:
            os.unlink(agents_path)
